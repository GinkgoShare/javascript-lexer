...
def cfgempty1(grammar,symbol,visited):
    if symbol not in [rule[0] for rule in grammar]:
        return [symbol]
    elif symbol in visited:
        return None
    else:
        new_visited = visited + [symbol]
        for rule in [rule[1] for rule in grammar if rule[0] == symbol]:
            trmls = []
            for pos in range(len(rule)):
                foo = cfgempty(grammar, rule[pos], new_visited)
                if foo != None:
                    trmls += foo
                else:
                    trmls += [foo]
            if all(trm != None for trm in trmls):
                return trmls
        return None

def cfgempty2(grammar,symbol,visited):
    if symbol not in [rule[0] for rule in grammar]:
        return [symbol]
    elif symbol in visited:
        return None
        ...
        new_visited = visited + [symbol]
        for rule in [rule[1] for rule in grammar if rule[0] == symbol]:
            if all(cfgempty(grammar, term, new_visited) != None for term in rule):
                trmls = []
                for term in rule:
                    trmls += cfgempty(grammar, term, new_visited)
                return trmls
        return None  
...
print cfgempty1(grammar1,"S",[]) == None 
...
print cfgempty1(grammar2,"S",[]) == ['c', 'd', 'b']
...
print cfgempty1(grammar3,"S",[]) == ['Barcelona', 'Las Palmas', 'Madrid', 'Huelva']
