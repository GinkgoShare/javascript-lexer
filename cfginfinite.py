...
def cfginfinite(grammar): 
    for rule in [rule for rule in grammar if len(rule[1]) > 1]: # filter to rules that have re-writes that contain more
                                                                # than one term (term can be terminal or non-terminal)
        for term in rule[1]:
            if term == rule[0]:
                return True
            elif term not in [rule[0] for rule in grammar]:
                continue
            else:
                foo = can_write_to(rule[0], grammar, term, [])
                if foo:
                    return foo
    return False
...
def can_write_to(static_term, grammar, dynamic_term, visited):
    if dynamic_term not in [rule[0] for rule in grammar]:
        return False
    elif dynamic_term in visited:
        return False
    else:
        new_visited = visited + [dynamic_term]
        for rule in [rule for rule in grammar if rule[0] == dynamic_term]:
            for term in rule[1]:
                if term == static_term:
                    return True
                else:
                    foo = can_write_to(static_term, grammar, term, new_visited)
                    if foo:
                        return foo
        return False

# Solution
def cfginfinite_sol(grammar):
...
print cfginfinite(grammar0) == True
